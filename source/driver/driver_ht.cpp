...
int main() {
    ...
    //Account myTeste[] =
    //{
    //   {"Paulo Vitor", 2, 1678, 54331, 1510.f}
    //};
    ...
    HashTbl< Account::AcctKey, Account, KeyHash, KeyEqual > copia;
    ...
    for( auto & e : myAccounts )
    {
        ...
        //std::cout << "ALOU → " << contas.retrieve( e.getKey(), conta_teste ) << std::endl; //PAULO
        //std::cout << "Conta_teste → " << conta_teste << std::endl; //PAULO
        //std::cout << "e → " << e << std::endl; //PAULO
     ...
     
    {
        ...
        // PAULO TESTANDO erase
        //std::cout << "Tamanho Da HashTabla → " << contas.size() << std::endl; //PAULO
        contas.erase(myAccounts[2].getKey());
        //std::cout << "Tamanho Da HashTabla → " << contas.size() << std::endl; //PAULO
        std::cout << "\n\n>>> Tabela Hash apos erase: \n" << contas << std::endl;

        // PAULO TESTANDO count
        //std::cout << "Quantidade de Elementos na Colisão → " << contas.count(myAccounts[0].getKey());
        //std:: cout << "Já Existe Na HashTable → " << contas.retrieve(myTeste[0].getKey(),myTeste[0]);
        // Verifica se está procurando a quantidade certa de itens em uma lista: contas.retrieve(myAccounts[0].getKey(),myAccounts[0]);
    }
    ...
    
    {
        ...
        std::cout << "\n>>> Apagando a tabela: \n";
        contas.clear();
        //std::cout << "Tamanho Table → " << contas.size() << std::endl;// PAULO
        std::cout << "\n\n>>> Tabela Hash apos limpar: \n" << contas << std::endl;
        assert( contas.empty() == true );

        //contas = copia; PAULO
        //std::cout << "\n\n>>> Tabela Hash apos copiar: \n" << contas << std::endl; PAULO
